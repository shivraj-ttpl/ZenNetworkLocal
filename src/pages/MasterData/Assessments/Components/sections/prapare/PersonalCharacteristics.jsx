...
import { LANGUAGE_OPTIONS, RACE_OPTIONS, ETHNICITY_OPTIONS } from "../../../constant";

const YES_NO_CHOOSE = ["Yes", "No", "I choose not to answer this question"];
const YES_NO = ["Yes", "No"];

export default function PersonalCharacteristics({ values, handleChange, setFieldValue }) {
  const pc = values?.personalCharacteristics || {};

  return (
    <div className="flex flex-col gap-6">
      <h3 className="text-base font-semibold text-text-primary">Personal Characteristics</h3>

      {/* 1. Race */}
      <div className="flex flex-col gap-2">
        <p className="text-sm font-medium text-text-primary">1. Are you Hispanic or Latino?</p>
      ...
      {/* 2. Race checkboxes */}
      <div className="flex flex-col gap-2">
        <p className="text-sm font-medium text-text-primary">2. Which race(s) are you?</p>
        {RACE_OPTIONS.map((opt) => (
          <Checkbox
            ...
            onChange={() =>
              setFieldValue(`personalCharacteristics.race.${opt.value}`, !pc.race?.[opt.value])
            }
          />
        ))}
        <div className="flex items-center gap-3">
          <Checkbox
            ...
            onChange={() =>
              setFieldValue("personalCharacteristics.race.other", !pc.race?.other)
            }
          />
          <Input
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            onChange={() =>
              setFieldValue("personalCharacteristics.race.chooseNotToAnswer", !pc.race?.chooseNotToAnswer)
            }
            className="w-4 h-4 accent-primary"
          />
      ...
      <div className="flex flex-col gap-2">
        <p className="text-sm font-medium text-text-primary">
          3. At any point in the past 2 years, has season or migrant farm work been your or your
      ...
      <div className="flex flex-col gap-2">
        ...
        <SelectDropdown
          ...
          onChangeCb={(val) => setFieldValue("personalCharacteristics.language", val)}
        />
      </div>
    </div>
  );
