...
const { updateProfile } = providerGroupProfileActions;
...
function buildEditInitialValues(data) {
  ...
  return {
    [FORM_FIELDS_NAMES.PROVIDER_GROUP_NAME]: data.name || '',
...
function handleSameAsPrimary(checked, values, setFieldValue) {
...
export default function EditProviderGroupDrawer() {
  ...
  const initialValues = useMemo(
    () => (editData ? buildEditInitialValues(editData) : null),
    [editData],
  );
  ...
  const handleFormSubmit = (values) => {
    ...
    dispatch(updateProfile({ id: editData.id, data }));
  };

  if (!drawerOpen || !initialValues) return null;
        ...
        {({
          ...
          <Form className="flex flex-col h-full">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-end gap-3">
                    <div className="flex-1">
                      <AsyncSelectDropdown
                        ...
                        url="dropdown-apis/provider-groups"
            ...
            <div className="flex justify-between gap-2 mt-auto pt-4 border-t border-[#E9E9E9]">
              ...
              >
                {isUpdating ? 'Updating...' : 'Update Provider Group'}
              </Button>
            </div>
          </Form>
        )}
